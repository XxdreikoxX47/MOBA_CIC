...
class AvatarJungla(Agente.Agente):
    ...
    def ataqueBasico(self, objetos_frame):
        ...
        while not self.boton1:                
            ...
            if len(self.target>0):
                ...
                objetos_frame[self.target].__dict__["salud"]-= 175+((175/self.nivel_max)*self.bonus_ataque)
    ...
    def ataqueEspecialBasico1(self, objetos_frame):
        ...
        if not self.boton2:                
            ...
            self.x += np.sign(addX)*4
            self.y += np.sign(addY)*4
            if len(targets)>0:
                for t in targets:
    ...
    def ganaExperiencia(self, exp):
        ...
        if self.experiencia >= (topeNivel) and self.nivel<self.nivel_max:
            self.nivel+=1
            self.bonus_ataque+=0.55
            self.salud_max = 9500*(1+(0.25*self.nivel))